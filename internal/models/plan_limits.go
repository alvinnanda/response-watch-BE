...
type PlanLimits struct {
	...
	PublicMonitoringLimit int // max items visible on public monitoring page
	SecureDesc            bool
	Reminder              bool
...
func GetPlanLimits(plan string) PlanLimits {
	limits := map[string]PlanLimits{
		...
		PlanEnterprise: {
			...
			VendorGroups:          0, // unlimited
			PublicMonitoringLimit: 0, // unlimited
			SecureDesc:            true,
			Reminder:              true,
			PublicMonitoring:      true,
		},
